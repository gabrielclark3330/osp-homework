...
/*
#define SIGALRM (SIGRTMIN)
#define SIGUSR1 (SIGRTMIN+1)
#define SIGTERM (SIGRTMIN+2)
*/
/*
...
static void suspend_worker(thread_info_t *info)
{
	...
	if(quanta > 0) {
	   ...
	   */
		...
		//TODO: might still need to update sched_queue?
	  /* Update Schedule queue */
...
void setup_sig_handlers() {
	...
	if (sigaction(SIGALRM, &act, NULL) == -1) {
		printf("SIGALRM failure");
		exit(EXIT_FAILURE);
	}

	/* Setup cancel handler for SIGTERM signal in workers */
	act.sa_flags = SA_SIGINFO;
	act.sa_sigaction = &cancel_thread;
	if (sigaction(SIGTERM, &act, NULL) == -1) {
		printf("SIGTERM failure");
		exit(EXIT_FAILURE);
	}
...
static void *scheduler_run(void *unused)
{
	...
	spec.it_value.tv_sec = QUANTUM;
	spec.it_value.tv_nsec = 0;
	spec.it_interval.tv_sec = QUANTUM;
	spec.it_interval.tv_nsec = 0;

	if (timer_settime(timer, CLOCK_REALTIME, &spec, NULL) == -1) {
		printf("Timer error");
	}
